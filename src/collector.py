...
def collect(ticker: str, retries: int = 3, base_pause: float = 2.0) -> StockData:
    ...
    if nv and nv.get("target_mean"):
        ...
        try:
            ...
            pass
    # 비(非)개별주(ETF·지수·채권·통화 등)는 실적·손익 데이터가 없어 yfinance가
    # 404/경고를 쏟으므로 건너뜀. quoteType이 비면 심볼로 ETF/지수 추정.
    qtype = str(raw.get("quoteType", "")).upper()
    is_equity = (qtype == "EQUITY"
                 if qtype else not ticker.startswith("^"))
    if is_equity:
        try:
            ...
            pass
    try:
        earnings = fetch_earnings(ticker, tk, fund) if is_equity else {}
