...
from sector import (ETF_LABELS, ROTATION_ETFS, SectorAnalyzer,  # noqa: E402
...
def _aclass(entry: dict) -> str:
    """워치리스트 항목의 자산군. 미지정/override는 equity로 간주."""
    return ((entry.get("profile") or {}).get("asset_class")) or "equity"
...
def run_full_analysis(tickers_override: list[str] | None = None) -> dict | None:
    ...
    # context(시장 벤치마크)는 점수 픽에서 제외하고 컨텍스트 패널로만 다룬다.
    # equity·tradable_etf만 종목 루프(랭킹) 대상.
    pick_entries = [e for e in entries if _aclass(e) != "context"]
    context_entries = [e for e in entries if _aclass(e) == "context"]
    tickers = [e["symbol"] for e in pick_entries]
    ...
    etf_set = needed_etfs(pick_entries)
    # 컨텍스트 패널 + tradable_etf 자기채점을 위해 해당 심볼 자체도 수집 대상에 포함.
    for e in context_entries + [x for x in pick_entries
                                if _aclass(x) == "tradable_etf"]:
        sym = e["symbol"]
        etf_set.setdefault(sym, ETF_LABELS.get(sym, e.get("name") or sym))
    ...
    # 시장 컨텍스트 패널 — context 자산(지수·채권·통화·변동성)의 추세 readout.
    # 점수 픽이 아니라 "지금 어떤 시장인가" 배경. SectorAnalyzer 메트릭 재사용.
    market_panel = []
    for e in context_entries:
        sym = e["symbol"]
        m = sectors.metrics.get(sym)
        if m is None:
            continue
        market_panel.append({
            "symbol": sym, "name": e.get("name") or sym,
            "sector": (e.get("profile") or {}).get("sector"),
            "chg_1d": m.chg_1d, "chg_5d": m.chg_5d, "chg_1m": m.chg_1m,
            "ma_alignment": m.ma_alignment, "above_ma200": m.above_ma200,
            "rsi": m.rsi,
        })
    if market_panel:
        print(f"      시장 컨텍스트 패널: {len(market_panel)}종 "
              + ", ".join(p["symbol"] for p in market_panel))
    ...
    for i, tk in enumerate(tickers):
        ...
        aclass = _aclass(entry)
        if aclass == "tradable_etf":
            sec = sectors.for_etf(tk)          # ETF 자기참조 섹터 객체
        else:
